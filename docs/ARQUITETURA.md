...
| Auth                  | **Supabase Auth**                                   | E-mail/senha, Google, reset, MFA prontos e battle-tested; espelho em `public.users` criado em código no 1º acesso (`ensureMirror`); funciona em qualquer host Node (Cloud Run incluso) via `@supabase/ssr` |
